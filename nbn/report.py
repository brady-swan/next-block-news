...
def render(con, day: str = None) -> str:
    ...
    out.append(
        ...
        f"<span class=pill>research: {_esc(config.HAIKU_RESEARCH_MODE)} · "
        f"{_esc(config.RESEARCH_MODEL)} @ {_esc(config.RESEARCH_EFFORT)}</span>"
        f"<span class=pill>desk every {config.DESK_INTERVAL_SECONDS // 60}m · "
        f"intake {config.DESK_CANDIDATE_MAX_AGE_HOURS:g}h</span>"
        f"<span class=pill>writer: {_esc(config.NEWSROOM_MODEL.replace('claude-', ''))}"
        f" @ {_esc(config.NEWSROOM_EFFORT)}</span>"
    ...
    out.append(
        ...
        f"<b>Native research</b> · {_bounded_count(usage.get('native_web_calls', 0), 10000)} web · "
        f"{_bounded_count(usage.get('native_x_calls', 0), 10000)} X calls · "
        f"{_bounded_count(usage.get('reasoning_tokens', 0), 100000000)} reasoning tokens "
        "(included in output)<br>"
        f"<b>Billing</b> · xAI reported totals; other calls rate-estimated · "
        f"{_bounded_count(usage.get('unknown_cost_calls', 0), 10000)} calls with unknown cost<br>"
    ...
    out.append(
        ...
        f"{prep['fail_open']} fail-open · {prep['sonnet_wakes_suppressed']} newsroom wakes "
        "suppressed<br>"
        f"<b>Preparation</b> · {int(prep_usage.get('calls', 0) or 0)} calls · "
        f"estimated ${float(prep_usage.get('estimated_cost_usd', 0) or 0):.4f}<br>"
        f"<b>Research assistant</b> · {int(research_assistant_usage.get('calls', 0) or 0)} calls · "
